...
const UrgentCareBooking = () => {
  ...
  // New state for blocked slots functionality
  const [showBlockModal, setShowBlockModal] = useState(false);
  const [selectedSlotForBlock, setSelectedSlotForBlock] = useState(null);
  const [blockReason, setBlockReason] = useState('');
  const [selectedDate, setSelectedDate] = useState('');
  
  // State for unblock confirmation modal
  const [showUnblockModal, setShowUnblockModal] = useState(false);
  const [selectedSlotForUnblock, setSelectedSlotForUnblock] = useState(null);
  ...
  const fetchTimeSlots = async () => {
    setLoading(true);
    try {
      // Get business info to determine the correct business timezone date
      const businessResponse = await fetch(`${API_BASE_URL}/api/business-info`);
      const businessData = await businessResponse.json();
      
      // Use business timezone (default to America/New_York if not configured)
      const businessTimezone = businessData.timezone || 'America/New_York';
      ...
      setSelectedDate(today); // Set the selected date
      ...
      // No need to fetch blocked slots separately anymore - they're included in the main API response
  ...
  const openBlockModal = (slotTime, slotDisplay) => {
    setSelectedSlotForBlock({ time: slotTime, display: slotDisplay });
    setBlockReason('');
  ...
  const handleBlockSlot = async () => {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/api/blocked-slots`, {
        ...
        body: JSON.stringify({
          date: selectedDate,
          time: selectedSlotForBlock.time,
          reason: blockReason || 'Staff blocked slot'
        })
      });
      ...
      if (response.ok) {
        ...
        closeBlockModal();
        // Refresh time slots (blocked slots info included)
        fetchTimeSlots();
      } else {
        const errorData = await response.json();
        setMessage({ type: 'error', text: errorData.detail || 'Failed to block slot' });
      }
    } catch (error) {
      console.error('Error blocking slot:', error);
      setMessage({ type: 'error', text: 'Unable to block slot. Please try again.' });
    } finally {
      setLoading(false);
    }
  };

  const handleUnblockSlot = async (slotId, slotTime) => {
    // Store slot info and show confirmation modal instead of browser confirm
    setSelectedSlotForUnblock({ id: slotId, time: slotTime });
    setShowUnblockModal(true);
  };

  // New function to handle the actual unblock after confirmation
  const confirmUnblockSlot = async () => {
    if (!selectedSlotForUnblock) return;

    setLoading(true);
    try {
      const response = await fetch(`${API_BASE_URL}/api/blocked-slots/${selectedSlotForUnblock.id}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('token')}`
        }
      ...
      if (response.ok) {
        ...
        const errorData = await response.json();
        setMessage({ type: 'error', text: errorData.detail || 'Failed to unblock slot' });
      }
    } catch (error) {
  ...
  const closeUnblockModal = () => {
    ...
    setSelectedSlotForUnblock(null);
  };
  ...
  return (
    <div className="min-h-screen bg-gray-50">
                  ...
                  <div>
                    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                      {timeSlots.map((slot) => {
                        ...
                        const blockedInfo = slot.blocked_info;
                        
                        return (
                          <div key={slot.value} className="relative">
                            {/* Time Slot Button */}
                            <button
                              onClick={() => !isBlocked && handleInputChange('appointment_time', slot.value)}
                              disabled={isBlocked && !isStaffMember}
                              className={`w-full p-4 text-center rounded-lg border-2 transition-all ${
                                isBlocked && !isStaffMember
                                ...
                                <div className="text-xs mt-1 flex items-center justify-center">
                                  ...
                                  {isStaffMember ? (
                                    <span className="text-red-600">
                                      Blocked{blockedInfo?.reason ? `: ${blockedInfo.reason.substring(0, 20)}${blockedInfo.reason.length > 20 ? '...' : ''}` : ''}
                                    </span>
                                  ) : (
                            ...
                            {/* Staff Controls - Only visible to managers and technicians */}
                            {isStaffMember && (
                              <div className="absolute top-2 right-2 flex space-x-1">
                                {isBlocked ? (
                                  <button
                                    onClick={() => handleUnblockSlot(blockedInfo.id, slot.display || formatTime(slot.time))}
                                    className="p-1 bg-green-500 hover:bg-green-600 text-white rounded-full transition-colors"
                                    title={`Unblock ${slot.display || formatTime(slot.time)}${blockedInfo?.reason ? ` - ${blockedInfo.reason}` : ''}`}
                                  >
                                    <Unlock className="h-3 w-3" />
                                  </button>
                                ) : (
                                  <button
                                    onClick={() => openBlockModal(slot.time, slot.display || formatTime(slot.time))}
                                    className="p-1 bg-red-500 hover:bg-red-600 text-white rounded-full transition-colors"
                                    title={`Block ${slot.display || formatTime(slot.time)}`}
                                  >
      ...
      {/* Block Slot Modal - Only visible to staff members */}
      {showBlockModal && isStaffMember && (
            ...
            <div className="space-y-4">
                ...
                <div className="flex items-center">
                  ...
                  <div>
                    <p className="text-sm font-medium text-yellow-800">
                      Block {selectedSlotForBlock?.display} slot
                    </p>
                    <p className="text-xs text-yellow-700">
                      This slot will not be available for online booking
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div>
                ...
                <textarea
                  ...
                  placeholder="e.g., Emergency surgery, Staff meeting, Equipment maintenance..."
                  rows="3"
      ...
      {/* Unblock Slot Confirmation Modal - Only visible to staff members */}
      {showUnblockModal && isStaffMember && (
        <div className="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50">
          <div className="relative top-20 mx-auto p-5 border w-full max-w-md bg-white rounded-lg shadow-lg">
            {/* Modal Header */}
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-lg font-medium text-gray-900">Unblock Time Slot</h3>
              <button
            ...
            <div className="space-y-4">
                ...
                <div className="flex items-center">
                  ...
                  <div>
                    <p className="text-sm font-medium text-green-800">
                      Unblock {selectedSlotForUnblock?.time} slot
                    </p>
                    <p className="text-xs text-green-700">
                      This slot will become available for online booking
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-gray-50 border border-gray-200 rounded-lg p-3">
                <p className="text-sm text-gray-700">
                  Are you sure you want to unblock this time slot? This action cannot be undone.
                </p>
              </div>
            </div>
