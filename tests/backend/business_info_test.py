...
# CRITICAL: Setup test database environment before any other imports
sys.path.append('/app/backend')
from test_db_config import set_test_environment, TestDatabaseManager
import asyncio

# Set test environment at module level
...
async def setup_test_database():
    ...
    try:
        ...
        await test_manager.setup_test_database()
...
def run_business_info_tests():
    ...
    print("="*60)
    print("🧪 BUSINESS INFORMATION API TESTS (TEST DATABASE)")
    print("="*60)
    print(f"🔒 Production database is PROTECTED")
    print(f"🧪 Using test database: {os.environ.get('DB_NAME')}")
    print("="*60)
    
    # Setup test database first
    if not asyncio.run(setup_test_database()):
        print("❌ Test database setup failed. Cannot proceed with tests.")
        return False
