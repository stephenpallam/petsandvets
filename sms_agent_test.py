...
class SMSAgentTester:
            ...
            if response.status_code == 200:
                ...
                if agent_id:
                    ...
                    if (agent_doc and agent_doc.get('agent_type') == 'sms_agent' and 
                        agent_doc.get('mode') == 'write' and
                        agent_doc.get('sms_content') == sms_agent_data['sms_content']):
    ...
    async def test_sms_agent_validation_missing_name(self):
        ...
        try:
            ...
            if response.status_code in [400, 500]:  # Accept both 400 and 500 for validation errors
                ...
                if "agent name is required" in error_text or "failed to create" in error_text:
                    self.log_test_result(
                        "SMS Agent Validation - Missing Agent Name", 
                        True, 
                        f"Correctly rejected (HTTP {response.status_code}): {response.text}"
                    )
                else:
                    self.log_test_result(
                        "SMS Agent Validation - Missing Agent Name", 
                ...
                self.log_test_result(
                    "SMS Agent Validation - Missing Agent Name", 
                    False, 
                    f"Expected 400/500 error, got {response.status_code}: {response.text}"
                )
    ...
    async def test_sms_agent_validation_character_limit(self):
        ...
        try:
            ...
            if response.status_code in [400, 500]:  # Accept both 400 and 500 for validation errors
                error_text = response.text.lower()
                if "character limit" in error_text or "160" in error_text or "failed to create" in error_text:
                    self.log_test_result(
                        "SMS Agent Validation - Character Limit", 
                        True, 
                        f"Correctly rejected long SMS (HTTP {response.status_code}): {response.text}"
                    )
                else:
                    self.log_test_result(
                        "SMS Agent Validation - Character Limit", 
                ...
                self.log_test_result(
                    "SMS Agent Validation - Character Limit", 
                    False, 
                    f"Expected 400/500 error, got {response.status_code}. Long SMS was accepted."
                )
